...
///
/// @brief Atomic max for float values (CUDA has no native float atomicMax)
///
/// Uses the __int_as_float / __float_as_int reinterpretation trick.
/// Works because IEEE 754 positive floats have the same ordering as their
/// integer bit-patterns when interpreted as signed ints, and we only ever
/// store non-negative values (|t|).
///
...
///
/// @brief FWER Pass 1 kernel: compute per-permutation max |t| across all connections
/// @param input        input buffer (subject values), layout: [conn][sub]
...
/// @param d_max_t      global array of size nr_perm, holds running max |t| per permutation
/// @param output_tstat output buffer for observed t-statistics (only from perm 0)
/// @param two_tailed   1 = two-tailed (use |t|), 0 = one-tailed (use directional |t|)
///
/// Each block processes one connection across ALL permutations.
/// For each permutation, computes |t|, then does atomicMaxFloat into d_max_t[perm].
/// The observed t-statistic (perm 0) is written to output_tstat.
///
__global__
                           ...
                           float *d_max_t, float *output_tstat, int two_tailed)
{
    ...
    extern __shared__ float smem[];  // [0] used for t_obs broadcast

    float t_obs = 0.f;

    // --- Observed statistic (permutation 0, thread 0 only) ---
    if (tid == 0)
    {
        ...
        // Observed |t| also participates in the max for permutation 0
        atomicMaxFloat(&d_max_t[0], fabsf(t_obs));
    }

    // Broadcast t_obs (needed for one-tailed directional extremeness)
    if (tid == 0) smem[0] = t_obs;
    __syncthreads();
    t_obs = smem[0];

    // --- Permuted statistics (permutations 1 .. nr_perm-1) ---
    for (int i = 1 + tid; i < nr_perm; i += block_size)
    {
        ...
        {
            float val = input[(n * nr_sub) + j];
            if (onehot[(i * nr_sub) + j] == 0) { b_sum += val; b_sq += val * val; nB++; }
            else                                { a_sum += val; a_sq += val * val; nA++; }
        }
        float a_mean = (nA > 0) ? a_sum / nA : 0.f;
        float b_mean = (nB > 0) ? b_sum / nB : 0.f;
        ...
        float b_var  = (nB > 1) ? (b_sq - b_sum * b_sum / nB) / (nB - 1.f) : 0.f;
        float se     = sqrtf(a_var / fmaxf(nA, 1.f) + b_var / fmaxf(nB, 1.f));
        float tstat  = (se > 1e-12f) ? (a_mean - b_mean) / se : 0.f;

        // For FWER max-statistic, we always track the absolute value |t|
        // regardless of one-tailed/two-tailed setting.
        // (The max-statistic null distribution is always built from |t_max|.)
        atomicMaxFloat(&d_max_t[i], fabsf(tstat));
    }
}

///
/// @brief FWER Pass 2 kernel: compute FWER-corrected p-values
/// @param input        input buffer (subject values), layout: [conn][sub]
/// @param onehot       permutations buffer in one-hot format (only perm 0 used)
/// @param nr_vals      number of connections in this chunk
/// @param nr_sub       number of subjects
/// @param nr_perm      number of permutations
/// @param d_max_t      max |t| null distribution array of size nr_perm
/// @param output_pval  output: FWER-corrected p-values
/// @param output_tstat output: observed t-statistics
/// @param two_tailed   1 = two-tailed, 0 = one-tailed
///
/// Each block processes one connection.
/// Re-computes observed t_obs (perm 0), then counts how many entries
/// in d_max_t are >= |t_obs| to get the FWER-corrected p-value.
///
__global__
                           ...
                           float *output_pval, float *output_tstat, int two_tailed)
{
    ...
    // --- Re-compute observed t_obs (perm 0) ---
    if (tid == 0)
    {
        float a_sum = 0.f, b_sum = 0.f;
        float a_sq  = 0.f, b_sq  = 0.f;
    ...
    __syncthreads();
    t_obs = smem[0];
    float abs_t_obs = fabsf(t_obs);

    // --- Count: how many max_t[perm] >= |t_obs|?  ---
    // Each thread handles a subset of permutations
    float local_count = 0.f;
    for (int i = tid; i < nr_perm; i += block_size)
    {
        if (d_max_t[i] >= abs_t_obs)
            local_count += 1.f;
    }
    ...
    if (tid == 0)
    {
        ...
        // FWER p-value: fraction of permutations where the global max |t|
        // was at least as extreme as this connection's |t_obs|.
        // No +1 correction needed here because perm 0 (observed) is
        // already included in d_max_t and will always count itself.
        float p_val = total / (float)nr_perm;
        output_pval[n] = p_val;
    }
}
