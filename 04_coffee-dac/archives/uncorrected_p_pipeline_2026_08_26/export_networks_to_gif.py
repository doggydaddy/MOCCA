...
def plotline_ijk(plotter, edge):
    ...
    plotter.add_mesh(spline, color=this_color, render_lines_as_tubes=True, line_width=3, opacity=0.2, show_scalar_bar=True)


# load edges with network clusterings
input_csv = 'edges_edclust.csv'
...
nr_network = int( np.max(edges_net[:, 7]) )
for i in range(nr_network):
    
    print("saving network ", i, " to gif.")
    ...
    # Open a gif
    output_name = filename + "_network_" + str(i) + ".gif"
    
    p.open_gif(output_name)

    # number of output frames
    nframe = 180

    # staring camera location
    p.camera.roll = 240
    p.camera.azimuth = 0
    p.camera.elevation = -60
    
    # update camera location for each frame
